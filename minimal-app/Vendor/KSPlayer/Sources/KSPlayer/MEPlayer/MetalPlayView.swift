            ...
            if fps != oldValue {
                if KSOptions.preferredFrame {
                    ...
                    if #available(iOS 15.0, tvOS 15.0, macOS 14.0, *) {
                        // videoClockSync can intentionally hold a frame until it is
                        // within half a source-frame of the audio clock. It therefore
                        // needs two display-link opportunities per source frame. The
                        // legacy path below already requests that cadence; preferring
                        // only the source FPS here can turn one hold into a missed
                        // frame and collapse 23.976 fps playback toward 12 fps.
                        let renderFramesPerSecond = 2 * preferredFramesPerSecond
                        displayLink.preferredFrameRateRange = CAFrameRateRange(
                            minimum: preferredFramesPerSecond,
                            maximum: renderFramesPerSecond,
                            __preferred: renderFramesPerSecond
                        )
                    } else {
