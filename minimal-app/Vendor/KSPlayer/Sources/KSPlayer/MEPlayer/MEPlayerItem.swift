...
public final class MEPlayerItem: Sendable {
    ...
    private var resolvedSeekByteOffset: Int64?
    private let seekResolutionLock = NSLock()
    private var seekResolutionGeneration: UInt64 = 0
...
//        }
        ...
        seekByBytes = (flags & AVFMT_NO_BYTE_SEEK == 0)
            && (flags & AVFMT_TS_DISCONT != 0)
            && options.formatName != "ogg"
            && !options.useTimeBasedSeekingForMPEGTS
    ...
    private func readThread() {
        ...
        while [MESourceState.paused, .seeking, .reading].contains(state) {
            ...
            if state == .seeking {
                ...
                let timeStamp: Int64
                let customByteOffset = resolvedSeekByteOffset
                resolvedSeekByteOffset = nil
                if let customByteOffset {
                    ...
                    timeStamp = customByteOffset
                    increase = customByteOffset - position
                } else if seekByBytes {
                    seekFlags |= AVSEEK_FLAG_BYTE
...
//            self.formatCtx?.pointee.pb = nil
    ...
    public func seek(time: TimeInterval, completion: @escaping ((Bool) -> Void)) {
        let generation = nextSeekResolutionGeneration()
        if options.formatName == "mpegts",
           let resolver = options.mpegTSByteSeekResolver {
            let duration = duration
            let completionBox = SeekCompletionBox(completion)
            Task { [weak self] in
                let byteOffset = await resolver(time, duration)
                guard let self,
                      self.isCurrentSeekResolution(generation)
                else { return }
                self.beginSeek(
                    time: time,
                    resolvedByteOffset: byteOffset,
                    completion: completionBox.callback
                )
            }
            return
        }
        beginSeek(time: time, resolvedByteOffset: nil, completion: completion)
    }

    private func beginSeek(
        time: TimeInterval,
        resolvedByteOffset: Int64?,
        completion: @escaping ((Bool) -> Void)
    ) {
        self.resolvedSeekByteOffset = resolvedByteOffset
    ...
    private func nextSeekResolutionGeneration() -> UInt64 {
        seekResolutionLock.withLock {
            seekResolutionGeneration &+= 1
            return seekResolutionGeneration
        }
    }

    private func isCurrentSeekResolution(_ generation: UInt64) -> Bool {
        seekResolutionLock.withLock {
            seekResolutionGeneration == generation
        }
    }
}
