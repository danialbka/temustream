...
open class KSOptions {
    ...
    /// Prefer FFmpeg's timestamp search over average-bitrate byte deltas for
    /// seekable MPEG-TS sources. This is intended for stable VOD transports;
    /// byte seeking remains the default for discontinuous/live inputs.
    public var useTimeBasedSeekingForMPEGTS = false
    /// Optional VOD index supplied by the host app. Returning an absolute byte
    /// position lets FFmpeg perform one range request instead of estimating a
    /// VBR seek from the container's average bitrate.
    public var mpegTSByteSeekResolver: (@Sendable (
        TimeInterval,
        TimeInterval
    ) async -> Int64?)?
    /*
