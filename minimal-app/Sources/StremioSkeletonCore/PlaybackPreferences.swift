...
enum VLCPlaybackBackend: Equatable, Sendable {
    case nativeHardware
    case compatibility
}

/// VLC remains the compatibility decoder for containers AVFoundation cannot
/// open. Apple-native streams take the system hardware path because
/// MobileVLCKit's HLS renderer can lose cadence even while VideoToolbox is
/// active, especially around transport-stream segment boundaries.
enum VLCPlaybackRouting {
    static func backend(
        for url: URL,
        detectedMIMEType: String? = nil
    ) -> VLCPlaybackBackend {
        let host = url.host?.lowercased() ?? ""
        let providerMayRelabelOrThrottlePayload = [
            "debrid", "torbox", "tb-cdn", "real-debrid", "alldebrid",
        ].contains(where: host.contains)
        if providerMayRelabelOrThrottlePayload {
